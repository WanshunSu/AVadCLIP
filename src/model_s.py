...
class AVADCLIP_MODEL(nn.Module):
    def __init__(self,
                 num_class: int,
                 embed_dim: int,
                 visual_length: int,
        ...
        self.fe_net = FE_Net(in_channel=512)
    ...
    def forward(self, visual, padding_mask, prompt_text, lengths):
        # prompt_text = ['a video of ' + label + ' event' for label in prompt_text]
        text_features_ori = self.encode_textprompt(prompt_text)

        visual_features = self.encode_video(visual, padding_mask, lengths)

        visual_features, theta = self.fe_net(visual_features)
        ...
        return visual_features, text_features_ori, logits1, logits2, theta, self.alpha