...
class UCFDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        clip_title = self.df.loc[index]['sentences']
        return clip_feature, clip_label, clip_title, clip_length

class XDDataset(data.Dataset):
    def __init__(self, clip_dim: int, clap_dim: int, file_path: str, test_mode: bool, label_map: dict):
        self.df = pd.read_csv(file_path)
        self.clip_dim = clip_dim
    ...
    def __getitem__(self, index):
        clip_feature = np.load(self.df.loc[index]['path'])
        clap_feature = np.load(self.df.loc[index]['audio_path'])
        if self.test_mode == False:
            clip_feature, clip_length = tools.process_feat(clip_feature, self.clip_dim)
            clap_feature, clap_length = tools.process_feat(clap_feature, self.clap_dim)
        else:
            clip_feature, clip_length = tools.process_split(clip_feature, self.clip_dim)
            clap_feature, clap_length = tools.process_split(clap_feature, self.clap_dim)

        clip_feature = torch.tensor(clip_feature)
        clip_label = self.df.loc[index]['label']
        return clip_feature, clap_feature, clip_label, clip_length, clap_length
